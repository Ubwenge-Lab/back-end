...
async function main() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    for (let j = 0; j < 2; j++) {
      ...
      const pharmacistUser = await prisma.user.create({
        data: {
          email: `pharmacist${i}_${j}@pharmacy.com`,
      ...
      const cashierUser = await prisma.user.create({
        data: {
          email: `cashier${i}_${j}@pharmacy.com`,
      ...
      await prisma.staffPermissions.create({
        data: { staffId: cashier.id, permissions: ['PROCESS_PAYMENTS'] },
      });
    }
  }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      const hStaffUser = await prisma.user.create({
        data: {
          email: `nurse${i}_${j}@hospital.com`,
          role: UserRole.NURSE,
          isVerified: true,
          password: passwordHash,
        },
    ...
    for (let j = 0; j < 5; j++) {
      const docUser = await prisma.user.create({
        data: {
          email: `doctor${i}_${j}@hospital.com`,
      ...
      const doctor = await (prisma.doctor.create as any)({
        data: {
  ...
  for (const branch of branches) {
    for (let i = 0; i < 10; i++) {
      ...
      const regItem = isRegistryLinked
        ? (faker.helpers.arrayElement(registryItems) as any)
        : null;
      ...
      const med = await prisma.medication.create({
        data: {
          ...
          name: isRegistryLinked
            ? regItem.brandName
            : faker.helpers.arrayElement(baseMeds) +
              ' ' +
  ...
  // NOTE: HospitalDrugStock seeding is skipped.
  ...
  for (let i = 0; i < 150; i++) {
    ...
    const orderMeds = faker.helpers.arrayElements(branchMeds, {
      min: 1,
      max: 3,
    });
