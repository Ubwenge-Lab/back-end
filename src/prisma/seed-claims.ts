...
async function main() {
  ...
  if (!hospital || !patient) {
    console.error(
      '❌ Error: Could not find any hospitals or patients. Please run the full database seed first:',
    );
  ...
  if (!doctor) {
    console.error(
      `❌ Error: Could not find any doctors for hospital ${hospital.name}. Please seed first.`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `🧑 Found Patient: ${patient.firstName} ${patient.lastName} (MRN: ${patient.mrn})`,
  );
  ...
  const testClaimsData = [
    {
      provider: 'RSSB-Test',
      amount: 15000,
      desc: 'Consultation & Triage Vitals',
    },
    {
      provider: 'MMI-Test',
      amount: 35000,
      desc: 'Dental Surgery & Post-Op Medications',
    },
    {
      provider: 'Sanlam-Test',
      amount: 8000,
  ...
  console.log(
    '🚀 Generating test Appointments, Invoices, and pending Insurance Claims...',
  );
  ...
  for (const item of testClaimsData) {
    ...
    console.log(
      `✅ Seeded Pending Claim for ${item.provider}: ${item.amount} RWF (Claim ID: ${claim.id})`,
    );
  }
