...
export class BranchesService {
  ...
  async sendCredentials(branchId: string, hqUserId: string) {
    ...
    if (branch.managerId)
      throw new BadRequestException('Credentials already sent');
    ...
    const existingUser = await this.prisma.user.findUnique({
      where: { email: branch.branchManagerEmail },
    });
    if (existingUser)
  ...
  async resendCredentials(branchId: string, hqUserId: string) {
    const branch = await this.validateBranchOwnership(branchId, hqUserId);
    if (!branch.managerId)
      throw new BadRequestException('Send credentials first');
    if (branch.branchStatus === BranchStatus.APPROVED)
      throw new ForbiddenException('Cannot resend for approved branch');
  ...
  async deleteBranch(branchId: string, hqUserId: string) {
    const branch = await this.validateBranchOwnership(branchId, hqUserId);
    if (branch.branchStatus === BranchStatus.APPROVED)
      throw new ForbiddenException('Cannot delete approved branch');
  ...
  private async validateBranchOwnership(branchId: string, hqUserId: string) {
    ...
    if (branch.pharmacyId !== pharmacy.id)
