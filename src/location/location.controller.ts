...
export class LocationController {
  ...
  @Get('verify')
  @ApiOperation({
    summary: 'Verify coordinates or fallback to Database address geopolygon',
  })
  ...
  async verifyFallback(
    ...
    const fallbackLocation =
      await this.locationService.verifyAndFallbackCoordinates(
        req.user.sub,
        req.user.role,
        lat,
        lon,
    ...
    if (!fallbackLocation) {
      return {
        success: false,
        message:
          'No active coordinates provided, and Database fallback address failed to yield a valid mapped location.',
        location: null,
      };
    }
    ...
    return {
      ...
      source:
        lat && lon && !isNaN(lat) ? 'ACTIVE_GPS' : 'DATABASE_FALLBACK_GEOCODED',
