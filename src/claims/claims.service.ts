...
import { ClaimStatus, HospitalBillingStatus, PaymentMethod, PaymentStatus } from '@prisma/client';
...
export class ClaimsService {
    ...
    async findAll(user: { sub: string; role: string }, filter: ClaimsFilterDto) {
        ...
        if (user.role === Role.HOSPITAL_ADMIN) {
            ...
            if (!hospital) {
                throw new NotFoundException('Hospital administrator does not have an associated hospital record.');
            }
    ...
    async reconcile(claimId: string, dto: ReconcileClaimDto) {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (!claim) {
                throw new NotFoundException(`Insurance claim with ID "${claimId}" not found.`);
            }

            if (claim.status === ClaimStatus.PAID) {
                throw new BadRequestException('This claim has already been fully reconciled and settled.');
            }
            ...
            return {
                success: true,
                message: nextClaimStatus === ClaimStatus.PARTIAL
