...
export class ClaimsController {
    ...
    @Get()
    @ApiOperation({ summary: 'List and filter insurance claims (Hospital Admin gets restricted list, Super Admin gets all)' })
    findAll(@Req() req: any, @Query() filter: ClaimsFilterDto) {
        return this.claimsService.findAll(req.user, filter);
    }
    @Patch(':claimId/reconcile')
    @ApiOperation({ summary: 'Record insurance company claim settlements and update invoice status atomically' })
    reconcile(
