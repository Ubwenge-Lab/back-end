...
export class GlobalExceptionFilter extends BaseExceptionFilter {
    ...
    constructor(
        ...
        let clientMessage: any = 'An unexpected error occurred. Please contact support.';
        if (exception instanceof HttpException) {
            const exceptionResponse = exception.getResponse();
            if (typeof exceptionResponse === 'object' && exceptionResponse !== null) {
                // If it's a validation error array, join it nicely, else extract the message
                const msg = (exceptionResponse as any).message;
                clientMessage = Array.isArray(msg) ? msg.join(', ') : msg || exceptionResponse;
            } else {
    ...
    private async sendEmailAlert(details: {
        ...
        const appName = this.configService.get<string>('RESEND_FROM_NAME') || 'E-Vuze Healthcare';
        const actionUrl = this.configService.get<string>('FRONTEND_URL') || 'http://localhost:3000';
